...
class Config:
    def __init__(self):
        ...
        self.wandb_name: str = "'gru test'" # wandb 내에서 작업 이름 설정 (중요)
        self.wandb_group: str = "'add rnn test" # wandb group.
        self.wandb_note: str = "'gru, num_layers=1, bias=True, batch_first=True, dropout=0.1, bidirectional=True'" # wandb note
        
        ## 2. 데이터 위치 (csv 파일)
        self.train_data_path: str = "../../dataset/train/train8/total_train.csv"
        self.val_data_path: str = "../../dataset/train/valid8/total_valid.csv"
        self.test_data_path: str = "../../dataset/test/test_data.csv"

        ## 3. 학습 설정
        '''
        train_type = {0: "base-model", 1: "rescent"}
        model_type = {0: "base-model", 1: "Masked_QA", 2: "MLM", 3: "R-BERT"}
